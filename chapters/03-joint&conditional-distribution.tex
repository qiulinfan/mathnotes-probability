...
$$
这个定义其实等价于: 对于任意的 Borel sets \(A, B\), 都有
$$
\mathbb{P}( A \mid B) = \mathbb{P}( A)  
$$
by definition of conditional probability.
它的意义是: \textbf{知道一个事件发生了, 对于另一个事件发生没有任何帮助.}
...
$$
这个定义等价于: 对于任意的 Borel sets \(A, B\), 都有
$$
\mathbb{P}(X \in A \mid Y \in B) = \mathbb{P}(X \in A)  
$$
by definition of conditional probability.
并且把 \(X\) 和 \(Y\) 的位置交换当然也等价 (
$
\mathbb{P}(Y \in B \mid X \in A) = \mathbb{P}(Y \in B)
$
). 

所以它实际上意义是: \textbf{知道一个 random variable 的值, 对于另外一个没有任何帮助. }

因而它们的 joint distribution 可以分解成 marginal distributions 的 product.
\end{remark}
...
\subsection{independence \(\implies\) expectation is closed under product}
