...
\subsubsection{discrete RV independence 的 characterization}
\begin{remark}
...
\subsubsection{continuous RV independence 的 geometric intuition}
...
\subsubsection{independence \(\implies\) expectation is closed under product}
...
\end{definition}
\begin{remark}
 Covariance 丈量两个 random variables 之间的 linear relationship: 
 对于一个事件,  \(X,Y\) 把它映射到的值是正相关的, 还是负相关的, 还是不相关的.
...
\begin{proposition}{covariance 的基本性质: as a symmetric bilinear form (and inner product)}
\begin{itemize}
    \item independence \(\implies\) zero covariance, 但是 zero covariance 不一定 \(\implies\) independence.
    \item RV sum 的 variance 可分解成 variances 和 covariance:   \[\text{Var}(X+Y) = \text{Var}(X) + \text{Var}(Y) + 2\text{Cov}(X,Y)\]
    ...
    \item covariance 满足 positive semi-definiteness: \(\text{Cov}(X,X) = \text{Var}(X) \geq 0\).
\end{itemize}
\end{proposition}
\begin{proof}
\begin{itemize}
    \item independence \(\implies\) 期望的乘积等于乘积的期望, 因而 covariance 是 0. 但是 zero covariance 不一定 \(\implies\) independence,
...
$$
\item $$
\operatorname{Cov}(a X+b, c Y+d)=\mathbb{E}[(a X+b-\mathbb{E}[a X+b])(c Y+d-\mathbb{E}[c Y+d])]=\mathbb{E}[(a X-\mathbb{E}[a X])(c Y-\mathbb{E}[c Y])]=a c \operatorname{Cov}(X, Y)
$$
\item by def.
\item by def.
\end{itemize}
\end{proof}
\begin{remark}
第一条的反例值得注意. 这说明了covariance 只能捕捉 linear relationship, 即趋势是否呈直线, 而不能捕捉 nonlinear relationship. 
比如, 如果 \((X,Y)\) 的联合分布 as a graph 是个圆圈, 那么 covariance 就是 0 而无法说明它们之间的关系.

为什么我们可以准确地说这是一种线性关系, 因为本质上, 我们既然说明了 covariance 是
(随机变量空间 $L^2(\Omega, \mathcal{F}, P)$ 上的)一个 symmetric bilinear form, 并且它显然满足 positive semi-definiteness:
\[
\text{Cov}(X,X) = \text{Var(X)} \geq 0
\]
那么它就是一个合法的 inner product on  $L^2(\Omega, \mathcal{F}, P)$. 
更加准确而言, 这是一个 \textbf{inner product on the quotient space} $L^2(\Omega, \mathcal{F}, P) / \sim$, 
其中 $\sim$ 是 equivalence relation: 所有只差一个 constant 的 random variables 
都被认为是 equivalent 的, 被减去了它们自己的均值, 移动到了原点上. 
(这个 space 还是 complete 的 since \(L^2\), 因而它其实是一个 Hilbert space. 原始的和 quotient 都是.)

原始 \(L^2\) space 的内积: \[\langle X, Y \rangle = \bE[XY]\]
因而自然而言, 我们 work on 的 quotient space 的内积就应该是 covariance:
 \[\langle X, Y \rangle_{\text{Cov}} = \bE[(X-\bE[X])(Y-\bE[Y])] = \text{Cov}(X,Y)\]

在这个空间上, 由 covariance 这个 inner product induce 出的 norm 即为一个
 random variable 的 standard deviation:
$$\|X\| = \sqrt{\text{Var}(X)} = \sigma_X$$

而两个 random variables \(X, Y\) 之间的夹角 \(\theta\) 的 cosine, 我们也专门做一个定义:
...
\begin{remark}
我们刚才将 covariance 作为 inner product, 是在 quotient space 
\(L^2(\Omega, \mathcal{F}, P) / \sim\) 上讨论; 而原始的 \(L^2\) space 
上的 inner product 是 \(\bE[XY]\). 这个原始 space 上的 Cauchy-Swartz 也值得一提: 

$$
\mathbb{E}\left[Z_1^2\right] \mathbb{E}\left[Z_2^2\right] \geq\left(\mathbb{E}\left[Z_1 Z_2\right]\right)^2
$$其中等号取得条件是 \(Z_1 = a Z_2\) for some \(a \in \bR\). \(a\) 
可以计算得是$$a = \frac{\mathbb{E}\left[Z_1 Z_2\right]}{\mathbb{E}\left[Z_2^ 2\right]}$$ 
