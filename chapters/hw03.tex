...
\]
Let $X=aZ+b$ where $a>0$. Then
\[
Z=\frac{X-b}{a}
\]
By the change-of-variables formula for densities,
\[
f_X(x)=f_Z\!\left(\frac{x-b}{a}\right)\cdot \frac{1}{a}.
\]
Hence
\[
f_X(x)
=\frac{1}{\sqrt{2\pi}}
\exp\!\left(-\frac{1}{2}\left(\frac{x-b}{a}\right)^2\right)
\cdot \frac{1}{a}
=\frac{1}{a\sqrt{2\pi}}
\exp\!\left(-\frac{(x-b)^2}{2a^2}\right).
\]

This is exactly the density of a normal distribution with mean $b$ and variance $a^2$. Therefore
\[
aZ+b \sim N(b,a^2).
\]

To compute mean and variance, use linearity:

\[
\mathbb E[aZ+b]=a\mathbb E[Z]+b=b,
\]
since $\mathbb E[Z]=0$, and
\[
\mathrm{Var}(aZ+b)=a^2\mathrm{Var}(Z)=a^2,
\]
since $\mathrm{Var}(Z)=1$.
\item [(b)] 
The standard normal density is
\[
\varphi(x)=\frac{1}{\sqrt{2\pi}}e^{-x^2/2},
\]
which is an even function: $\varphi(x)=\varphi(-x)$. Thus,
\[
\Phi(0)=\int_{-\infty}^{0}\varphi(x)\,dx
=\int_{0}^{\infty}\varphi(x)\,dx.
\]
Since $\int_{-\infty}^{\infty}\varphi(x)\,dx=1$, the two equal halves must each be $1/2$, so $\Phi(0)=1/2$.
\item [(c)]
For any $x\in\mathbb R$,
\[
\Phi(-x)=\int_{-\infty}^{-x}\varphi(t)\,dt.
\]
Let $u=-t$ (so $dt=-du$). Using $\varphi(-u)=\varphi(u)$,
\[
...
\end{cases}
\]
Note that on both rectangles we have $-xy>0$.
On $(1,2)\times(-1,0)$ we have $x+y>0$ (since $x>1$ and $y>-1$), hence this region contributes nothing to $\{X+Y<0\}$.
On $(-1,0)\times(0,1)$, the inequality $x+y<0$ is equivalent to $0<y<-x$. Therefore,
\[
...
By definition,
\[
\mathbb E[XY]=\iint xy\,f(x,y)\,dx\,dy=-\iint x^2y^2\,dx\,dy,
\]
where the integral is over the support. Split over the two rectangles.
On $(-1,0)\times(0,1)$,
\[
...
\]
\item [(c)]
Compute marginals. For $x\in(-1,0)$,
\[
f_X(x)=\int_{0}^{1}(-xy)\,dy=\frac{-x}{2},
\]
and for $x\in(1,2)$,
\[
f_X(x)=\int_{-1}^{0}(-xy)\,dy=\frac{x}{2},
\]
otherwise $f_X(x)=0$.
For $y\in(0,1)$,
\[
...
and for $y\in(-1,0)$,
\[
f_Y(y)=\int_{1}^{2}(-xy)\,dx=(-y)\int_{1}^{2}x\,dx=\frac{-3y}{2},
\]
otherwise $f_Y(y)=0$.
Now for $(x,y)\in(-1,0)\times(0,1)$,
\[
...
\begin{solution}
Let $X\sim \mathrm{Exp}(1)$ and $Y=X+\frac{1}{X+1}$.

\textbf{1) Compute $\mathbb P((X+1)Y\le 2)$.}
Observe that
\[
...
\[
0\le X\le r,\qquad r:=\frac{\sqrt5-1}{2}.
\]
Therefore, using the CDF of $\mathrm{Exp}(1)$,
\[
\mathbb P((X+1)Y\le 2)=\mathbb P(X\le r)=1-e^{-r}
=1-\exp\!\left(-\frac{\sqrt5-1}{2}\right).
\]

\textbf{2) Compute $\mathrm{Cov}(X,Y)$.}
\[
...
\]
First compute the marginal density of $X$. For $x\ge 0$,
\[
...
\end{itemize}
\begin{solution}
Let $p$ be the (random) head probability, with $p\sim \mathrm{Unif}(0,1/2)$. Thus the density of $p$ is
\[
f_P(p)=2\,\mathbf 1_{(0,1/2)}(p).
\]

\textbf{(a)} The unconditional probability of heads on the first toss is
\[
...
\textbf{(b)} Let $T$ be the number of tosses until the first tail occurs. Conditional on $p$, tails occurs with probability $1-p$ each toss, so $T$ is geometric with parameter $1-p$ (counting the trial of the first success). Hence
\[
...
f_{X,Y}(x,y)=
\begin{cases}
c\left(x^2+\dfrac{xy}{2}\right), & (x,y)\in(0,1)\times(0,2),\\
0, & \text{otherwise.}
\end{cases}
\]

\textbf{(a)} Determine $c$ from normalization:
\[
...
\textbf{(b)} The marginal density of $X$ (for $0<x<1$) is
\[
f_X(x)=\int_0^2 c\left(x^2+\frac{xy}{2}\right)\,dy=c(2x^2+x)=\frac{6}{7}(2x^2+x),
\]
and $f_X(x)=0$ otherwise. Then
\[
...
\textbf{(c)} The event $\{X>Y\}$ corresponds to the region $0<y<x<1$ (since $x\in(0,1)$). Hence
\[
