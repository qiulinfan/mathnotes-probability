...
\item 对于任意 \(x_1, \cdots, x_n\), 有 
\(\bP(X_1 = x_1
, \cdots,X_n = x_n) = 0\)
\end{itemize}
\end{proposition}
\begin{proof}
    由于 \(f_{\mathbf{X}}\) 是 \(\bP^{\mathbf{X}}\) 对于 \(\lambda^n\) 的 Radon-Nikodym derivative, 
...
\begin{proposition}{计算 marginal distribution 的方法}
如果 \(\mathbf{X}\) 是 continuous 的, 那么任意 \(X_i\) 也是 continuous random variable 
(即其 cdf 是 absolutely continuous 的), 
并且: \[
